...
For example : $X = \{1,2,3,1,2,5\}$  $X = {x^{(1)},x^{(2)},\cdots}$ and $x^{(1)}$ is the first element $1$ ,$x^{(2)}$ is the second element $2$ and so on . There will be $x^{(1)},\cdots,x^{(6)}$ six elements ,but the $x$ is can only take four value 1,2,3,5 ,which is four . $\#1$ and $\#2$ both are 2,and anothers are 1 .
